@echo off
echo ========================================
echo БЕЗОПАСНАЯ загрузка изменений в GitHub
echo ========================================
...
echo.
echo 7. Пытаемся получить изменения с GitHub...
git pull origin main --allow-unrelated-histories --no-edit

if %errorlevel% neq 0 (
    echo.
    echo ВНИМАНИЕ: Возникли конфликты при слиянии!
    echo.
    echo Выберите действие:
    echo 1. Принудительно отправить изменения (перезаписать удаленные)
    ...
    if "!choice!"=="1" (
        echo.
        echo Принудительно отправляем изменения...
